...
const emit = defineEmits<{
  save: [config: { name: string; url: string; headers: Record<string, string>; events: string[] }];
  cancel: [];
}>();
  ...
  async (vis) => {
    if (vis) {
      ...
      if (props.integration) {
        ...
        url.value = cfg.url ?? "";
        headers.value = Object.entries(cfg.headers ?? {}).map(([k, v]) => ({
          key: k,
          value: v,
        }));
  ...
  <Teleport to="body">
    <Transition name="modal">
      <div v-if="visible" class="config-overlay" @mousedown="onOverlayClick">
        <div ref="modalRef" class="config-modal" role="dialog" aria-label="Webhook Configuration">
          <h2 class="config-title">{{ integration ? "Edit Webhook" : "New Webhook" }}</h2>
          ...
          <div class="form-field">
            ...
            <div v-for="(h, i) in headers" :key="i" class="header-row">
              ...
              <input
                ...
                placeholder="Value"
              />
              <button class="remove-header" type="button" @click="removeHeader(i)">
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
            ...
            <button class="add-header-btn" type="button" @click="addHeader">+ Add Header</button>
          </div>
